@foreach ($comments as $comment)
    <div class="grid grid-cols-1 gap-4 mt-3 mb-3">
        <div id="comment_0212{{ $comment->id }}" data-userId="{{ $comment->user->id }}" data-user="{{ $comment->user->username }}" data-comment="{{ Str::limit($comment->content, 15) }}" class="p-3 rounded-lg bg-opacity-40 bg-neutral">
