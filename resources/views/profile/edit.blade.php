                        ...
                        <form class="p-2 mt-2" method="post" action="{{ route('profile.update') }}">
                            ...
                            <div class="form-group">
                                ...
                                <input type="email" class="form-control" name="email" id="email"
                                    placeholder="Enter a valid e-mail" value="{{ old('email', $user->email) }}" required />
                                @error('email')
                                    <span class="text-danger" role="alert">
                                        {{ $message }}
                                    </span>
                                ...
                                <div>
                                    <p class="mt-2 text-sm">
                                        ...
                                        <button form="send-verification"
                                            class="btn btn-sm btn-info">{{ __('Click here to re-send the verification email.') }}</button>
                                    </p>
                            ...
                            <div class="flex-row d-flex align-items-baseline ">
                                ...
                                @if (session('status') === 'profile-updated')
                                    <p class="p-2 text-sm text-muted">{{ __('Saved.') }}</p>
                                @endif
                            </div>
                        </form>
                    </div>
                        ...
                        <form class="p-2 mt-2" method="post" action="{{ route('password.update') }}">
                            ...
                            <div class="form-group">
                                ...
                                <input type="password" class="form-control" name="current_password"
                                    id="update_password_current_password" placeholder="Current password" required />
                                @if ($errors->updatePassword->has('current_password'))
                                    <p class="p-2 text-sm text-danger">{{ __('Incorrect Password!') }}</p>
                                @endif
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                @if ($errors->updatePassword->has('password'))
                                    <p class="p-2 text-sm text-danger">
                                        {{ __('The password field must be at least 8 characters') }}</p>
                                @endif
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="password" class="form-control" name="password_confirmation"
                                    id="update_password_password_confirmation" placeholder="Confirm new password"
                                    required />
                                @if ($errors->updatePassword->has('password_confirmation'))
                                    <p class="p-2 text-sm text-danger">{{ __('Password does not match') }}</p>
                                @endif
                            </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="modal fade" id="confirm-user-deletion" tabindex="-1"
                            aria-labelledby="confirm-user-deletionLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel">{{ __('Confirm account deletion') }}
                                        </h5>
                                    ...
                                    <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
                                        ...
                                        <div class="modal-body">
                                            <p><strong>{{ __('Are you sure you want to delete your account?') }}</strong>
                                            </p>
                        ...
                        @if ($errors->userDeletion->has('password'))
                            <p class="p-2 text-sm text-danger">{{ __('Failed to delete account. Incorrect Password!') }}
                            </p>
                        @endif
